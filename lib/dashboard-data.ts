...
export async function getDashboardData(userId: string) {
  ...
  const flatExpenses: (DashboardGroup["expenses"][number] & {
    rawDate: Date;
  })[] = [];
  ...
  const groups: DashboardGroup[] = memberships.map(({ group }) => {
    ...
    for (const e of group.expenses) {
      ...
      for (const s of e.shares)
        netMap.set(s.userId, (netMap.get(s.userId) ?? 0) - Number(s.amount));
    }
    ...
    while (i < creditors.length && j < debtors.length) {
      ...
      if (creditors[i][0] === userId || debtors[j][0] === userId) {
        const otherId =
          creditors[i][0] === userId ? debtors[j][0] : creditors[i][0];
    ...
    const expenseRows = group.expenses.map((e) => ({
      ...
      title: e.title,
      paid:
        e.paidById === userId ? "You paid" : `${firstName(e.paidBy.name)} paid`,
