...
type ExpenseRow = DashboardGroup['expenses'][number];
type Props = {
...
export default function SplitEasyApp({ currentUserId, currentUserName, groups, recentExpenses, overall, debtLines }: Props) {
  ...
  const [selectedGroup, setSelectedGroup] = useState<DashboardGroup | null>(groups[0])
    ...
    <div className="min-h-screen bg-[#f8f7f4] text-[#28252a]">
      ...
      <div className="lg:pl-[238px]">
        ...
        <main className="mx-auto max-w-[1180px] px-5 py-8 sm:px-8 lg:px-12 lg:py-12">
          ...
          {view === 'balance' && <Balance debtLines={debtLines} onBack={() => navigate('overview')} />}
...
function Overview({ overall, groups, recentExpenses, onAdd, onNew, onGroup }: {
  overall: Props['overall']; groups: DashboardGroup[]; recentExpenses: ExpenseRow[]; onAdd: () => void; onNew: () => void; onGroup: (group: DashboardGroup) => void
}) {
  return <>
    ...
    <section className="mt-9 grid gap-4 md:grid-cols-3"><BalanceCard label="You are owed" amount={formatINR(overall.owed)} detail={`Across ${groups.filter((g) => g.status === 'owed').length} group(s)`} icon={<ArrowDownLeft />} tone="green" /><BalanceCard label="You owe" amount={formatINR(overall.owe)} detail="Across 1 group" icon={<ArrowUpRight />} tone="red" /><BalanceCard label="Total shared spend" amount="₹54,550" detail="Across 3 groups" icon={<Receipt />} tone="neutral" /></section>
...
function GroupCard({ group, onClick }: { group: DashboardGroup; onClick: () => void }) { return <button onClick={onClick} className="group rounded-2xl border border-[#e8e4df] bg-[#fbfaf8] p-5 text-left transition hover:-translate-y-0.5 hover:border-[#d7cec6] hover:shadow-lg hover:shadow-[#ded7d0]/30"><div className="flex items-start justify-between"><div className={`flex size-10 items-center justify-center rounded-xl text-xl ${group.tone === 'peach' ? 'bg-[#ffe5da]' : group.tone === 'lilac' ? 'bg-[#e9e5ff]' : 'bg-[#daf4e7]'}`}>{group.tone === 'peach' ? '◒' : group.tone === 'lilac' ? '⌂' : '✦'}</div><MoreHorizontal className="size-5 text-[#aaa5a0]" /></div> bbbbb<h3 className="mt-5 text-lg font-bold tracking-[-0.03em]">{group.name}</h3><p className="mt-1 text-xs text-[#aaa5a0]">{group.subtitle}</p><div className="mt-7 flex items-end justify-between"><div><p className="text-[11px] text-[#aaa5a0]">Total spend</p><p className="mt-1 text-[17px] font-bold">{group.total}</p></div><div className="text-right"><p className="text-[11px] text-[#aaa5a0]">Your balance</p><p className={`mt-1 text-[17px] font-bold ${group.status === 'owed' ? 'text-[#3e9b6c]' : group.status === 'owe' ? 'text-[#d87452]' : 'text-[#777177]'}`}>{group.balance}</p></div></div><div className="mt-5 flex items-center justify-between"><div className="flex -space-x-2">{group.members.map((m, i) => <Avatar key={m} initials={m} index={i} />)}</div><span className="flex items-center gap-1 text-xs font-semibold text-[#aaa5a0] group-hover:text-[#f07d58]">Open group <ChevronRight className="size-3.5" /></span></div></button> }
